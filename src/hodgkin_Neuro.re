...
let draw2d =
  ...
  Util.withContext(
    () => {
      ...
      let neuroColor = colorOf(Utils.norm(~value=varColor, ~low=minColor, ~high=maxColor));
      Util.withContext(
        () => {
          Draw.fill(neuroColor, env);
          let scale = max(0., varPulse);
          let size = size *. 0.1 +. size *. Utils.lerpf(~low=0., ~high=0.9, ~value=scale);
          Draw.ellipsef(~center=(0., 0.), ~radx=size /. 2., ~rady=size /. 2., env)
        },
        env
      );
...
let stepEuler =
    (~t, f: array(float) /* state array */ => array(float) /* deriv array */, st: array(float))
    : array(float) => {
  let d = f(st);
...
let add_arrays = (arr1, arr2) => Array.mapi((i, arr1_i) => arr1_i +. arr2[i], arr1);

let mult_arrays = (arr1, arr2) => Array.mapi((i, arr1_i) => arr1_i *. arr2[i], arr1);
...
let stepExplicitRungeKutta =
  ...
  weights |> Array.iteri((i, w_i) => acc := add_arrays(acc^, mult_array_constant(k[i], w_i)));
